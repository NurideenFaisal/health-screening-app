...
export default function ClinicianDashboard() {
  const { profile, user, clearAuth, loadSession: storeLoadSession } = useAuthStore()
  const navigate = useNavigate()

  // Load session on mount if missing
  useEffect(() => {
    if (!user && typeof storeLoadSession === 'function') {
      storeLoadSession()
    }
  }, [user, storeLoadSession])
  ...
  async function handleLogout() {
    ...
    try {
      ...
      // clear local store if provided (support sync or async)
  ...
  const links = ['Dashboard', 'Patient Data', 'Screen Form']
