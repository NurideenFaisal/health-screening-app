    ...
    <div className="bg-white rounded-xl shadow border border-slate-200 overflow-hidden">
      <table className="w-full text-sm">
        <thead className="bg-slate-50 border-b-2 border-slate-200">
          <tr>
            <th className="text-left p-4 font-semibold text-slate-600 w-[30%]">Name</th>
            <th className="text-left p-4 font-semibold text-slate-600 w-[15%]">Role</th>
            <th className="text-left p-4 font-semibold text-slate-600 w-[30%]">Sections</th>
            <th className="text-left p-4 font-semibold text-slate-600 w-[10%]">Status</th>
            <th className="text-right p-4 font-semibold text-slate-600 w-[15%]">Actions</th>
          </tr>
        </thead>
        <tbody>
          {filtered.map(u => {
            ...
            return (
              <tr key={u.id} className={`border-b border-slate-100 hover:bg-slate-50 transition-colors ${isDisabled ? 'opacity-50' : ''}`}>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className={`w-8 h-8 rounded-full flex-shrink-0 ${getRoleColor(u.role)} flex items-center justify-center font-semibold text-xs`}>{getInitial(u.full_name)}</div>
                    <span className="font-medium text-slate-800 text-sm truncate">{u.full_name}</span>
                  </div>
                </td>
                <td className="px-4 py-3">
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-0.5">
                    <button onClick={() => onEdit(u)} className="p-2 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition" title="Edit"><Edit2 size={14} /></button>
                    <button onClick={() => onReset(u)} className="p-2 text-slate-400 hover:text-amber-500 hover:bg-amber-50 rounded-lg transition" title="Reset password"><KeyRound size={14} /></button>
                    <button onClick={() => onDelete(u)} className={`p-2 rounded-lg transition ${isDisabled ? 'text-emerald-500 hover:text-emerald-700 hover:bg-emerald-50' : 'text-red-400 hover:text-red-600 hover:bg-red-50'}`} title={isDisabled ? 'Activate' : 'Deactivate'}>
